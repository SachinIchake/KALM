...
parser.add_argument('--data', type=str, default='data/CoNLL-2003/',
...
model = model.RNNModel(args.model, ntokens, args.emsize, args.nhid, args.nlayers, args.dropout, args.dropouth, args.dropouti, args.dropoute, args.wdrop, args.tied,args.bptt)
###
if args.resume:
...
def train():
    ...
    ntokens = len(corpus.dictionary)
    hidden,w_e,vt_1 = model.init_hidden(args.batch_size)
    batch, i = 0, 0
    while i < train_data.size(0) - 1 - 1:
        # bptt = args.bptt if np.random.random() < 0.95 else args.bptt / 2.
        # Prevent excessively small or negative sequence lengths
        seq_len = args.bptt #max(5, int(np.random.normal(bptt, 5)))
        ...
        output, hidden, rnn_hs, dropped_rnn_hs,vt_1 = model(data, hidden,w_e,vt_1, return_h=True)
